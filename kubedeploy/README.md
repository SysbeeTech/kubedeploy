...
| initContainer.command | list | `["exit","0"]` | Define custom command for initContainer to run |
| initContainer.enabled | bool | `false` | define if we should deploy init container within a pod |
| initContainer.pullPolicy | string | `"IfNotPresent"` | default initContainer pull policy |
| initContainer.repository | string | `"busybox"` | define initContainer repository |
| initContainer.resources | object | `{}` | Define init container resources |
| initContainer.tag | string | `""` | Overrides the image tag whose default is the chart appVersion. |
